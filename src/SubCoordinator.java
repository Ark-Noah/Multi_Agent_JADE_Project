	...
	public class ReceiveMessage extends CyclicBehaviour {
		...
		public void action() {
			...
			if (msg != null) {
				...
				System.out.println("***The subcoordinator Received a Message***" + senderName + ": " + msgContent);
				...
				if (action.contains("Create")) {
					...
					for (int i = 0; i < Integer.parseInt(NumberOfAgents); i++) {
						try {
							String S = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss.SSS")
									.format(new Timestamp(System.currentTimeMillis()));

							AgentController t1 = container.createNewAgent(basename + S + "-" + i, "AgentSmith", args);
							t1.start();
							System.out.println(getLocalName() + " CREATED AND STARTED NEW SMITH:" + basename + "-" + i
